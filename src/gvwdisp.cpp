...
#ifdef OLD
/* Don't use these because they trip a bug in VC++ 5.0:
 * fstatus.st_mtime gives different times on main and secondary
 * thread on days when daylight savings changes occur.
 */
...
struct stat fstatus;
	...
	if (debug & DEBUG_GENERAL) {
	    char buf[256];
	    sprintf(buf, "psfile_changed: length=%ld time=%ld \042%s\042\n",
	    	fstatus.st_size, fstatus.st_mtime, ctime(&fstatus.st_mtime));
	    gs_addmess(buf);
	}
...
struct stat fstatus;
	   ...
	   psf = &psfile;
	fstat(fileno(psf->file), &fstatus);
	GetFileTime((HANDLE)_get_osfhandle(fileno(psf->file)), 
		NULL, NULL, &thisfiletime);
	thisflength = fstatus.st_size;
	if (debug & DEBUG_GENERAL) {
	    char buf[256];
	    sprintf(buf, "psfile_changed: length=%lu time=%ld %ld \042%s\042\n",
	    	fstatus.st_size, 
		thisfiletime.dwHighDateTime, thisfiletime.dwLowDateTime,
		text_filetime(&thisfiletime));
	    gs_addmess(buf);
	}
	return ( (thisflength != psf->length) ||
		CompareFileTime(&thisfiletime, &(psf->filetime)) );
}
...
struct stat fstatus;
	fstat(fileno(psf->file), &fstatus);
	GetFileTime((HANDLE)_get_osfhandle(fileno(psf->file)), 
		NULL, NULL, &(psf->filetime));
	psf->length = fstatus.st_size;
	if (debug & DEBUG_GENERAL) {
	    char buf[256];
	    sprintf(buf, "psfile_changed: length=%lu time=%ld %ld \042%s\042\n",
	    	fstatus.st_size, 
		psf->filetime.dwHighDateTime, psf->filetime.dwLowDateTime,
		text_filetime(&(psf->filetime)));
	    gs_addmess(buf);
	}
}
#endif
