...
APIRET rc;
    ...
    rc = DosLoadModule(buf2, sizeof(buf2), buf, &unzip_hinstance);
    if (rc==0) {
	if ((rc = DosQueryProcAddr(unzip_hinstance, 0, "UzpAltMain", (PFN *)(&UnzipAltMain)))!=0) {
	    free_unzip();
	}
	else {
	    if ((rc = DosQueryProcAddr(unzip_hinstance, 0, "UzpVersion", (PFN *)(&UnzipVersion)))!=0) {
		free_unzip();
	    }
	}
    }
