...
const char *depthlist[] =    {"Default", "1"};
int index_to_depth[] = { 0,         1};
...
void cfg_defaults(GtkWidget *w, gpointer data)
{
    ...
    gtk_entry_set_text(GTK_ENTRY(entry->gsexe), "gs");
...
install_gsdll(void)
{
    ...
    /* Ghostscript EXE */
    gsexe_label = gtk_label_new("Ghostscript Program:");
    ...
    gtk_entry_set_text(GTK_ENTRY(entry.gsexe), option.gsdll);
	...
	strncpy(option.gsdll, gtk_entry_get_text(GTK_ENTRY(entry.gsexe)),
	    sizeof(option.gsdll));
