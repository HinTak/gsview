...
gsview_text_extract_quick()
{
	...
	for (i = 0; i < (int)psfile.dsc->page_count; i++) {
		...
		fseek(psfile.file, psfile.dsc->page[map_page(i)].begin, SEEK_SET);
		text_extract(f, psfile.file, psfile.dsc->page[map_page(i)].end);
		fputc('\f', f);
		fputc('\n', f);
	    }
	}
...
char *p;
	...
	if (psfile.page_list.select[map_page(i)])  {
	    ...
	    fseek(psfile.file, psfile.dsc->page[map_page(i)].begin, SEEK_SET);
	    p = text_find_section(psfile.file, psfile.dsc->page[map_page(i)].end, szFindText);
	    if (p) {	/* found it */
		info_wait(IDS_NOWAIT);
		free(p);
		dfclose();
