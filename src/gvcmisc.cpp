...
info_init(HWND hwnd)
{
	...
	if (dsc) {
	    ...
	    *p = '\0';
	    ...
	    SetDlgItemText(hwnd, INFO_TITLE, dsc->dsc_title ? dsc->dsc_title : "");
	    SetDlgItemText(hwnd, INFO_DATE, dsc->dsc_date ? dsc->dsc_date : "");
	    if (dsc->bbox)
...
PROFILE *prf;
    ...
    for (i=0; i<NUMSOUND; i++) {
	    ...
	    strncpy(sound[i].file, profile, sizeof(sound[i].file)-1);
    }
    profile_read_string(prf, section, "LastFile1", "", last_files[0], sizeof(last_files[0]));
    profile_read_string(prf, section, "LastFile2", "", last_files[1], sizeof(last_files[1]));
    profile_read_string(prf, section, "LastFile3", "", last_files[2], sizeof(last_files[2]));
    profile_read_string(prf, section, "LastFile4", "", last_files[3], sizeof(last_files[3]));
