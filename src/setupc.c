...
char buf[MAXSTR];
int ver;
    GetDlgItemText(hwnd, IDD_GSVER_TEXT, buf, sizeof(buf));
    if (strlen(buf) == 4)
...
BOOL fixit = FALSE;
	...
	SetDlgItemText(hwnd, IDD_GSVER_TEXT, buf);
    ...
    gsver = ver;
    install_init();
    SetDlgItemText((find_page_from_id(IDD_DIR)->hwnd), IDD_DIR_GSVER, gs_basedir);
    return 0;
}
...
int i;
WIZPAGE *page;
    GetDlgItemText(hwnd, ID_ANSWER, destdir, sizeof(destdir));
    page = find_page_from_id(IDD_MKDIR);
    if (page)
	SetDlgItemText((page->hwnd), IDD_MKDIR_TEXT, destdir);
