...
// FIX THIS - doesn't handle convert properly
#ifdef __BORLANDC__
#pragma argsused
#endif
BOOL CALLBACK _export
PageRangeDlgProc(HWND hDlg, UINT wmsg, WPARAM wParam, LPARAM lParam)
{
	...
	if ((psfile.dsc!=(CDSC *)NULL) || print_method==PRINT_GDI) {
	    char buf[32];
	    wsprintf(buf, "%d", psfile.print_from);
	    SetDlgItemText(hDlg, IDC_FROM, buf);
	    wsprintf(buf, "%d", psfile.print_to);
	    SetDlgItemText(hDlg, IDC_TO, buf);
	}
	else {
...
get_page_range(HWND hwnd, int method)
{
    ...
    if (DialogBoxParam(hlanguage, "PageRangeDlgBox", hwnd, 
...
NewDeviceDlgProc(HWND hDlg, UINT wmsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (SendDlgItemMessage(hDlg, DEVICE_NAME, CB_SELECTSTRING, 0, 
	    (LPARAM)(LPSTR)(s)) == CB_ERR) {
	    if (strlen(s))
		SetDlgItemText(hDlg, DEVICE_NAME, s);
	    else
	...
	if (SendDlgItemMessage(hDlg, DEVICE_RES, CB_SELECTSTRING, 0, 
	    (LPARAM)(LPSTR)(s)) == CB_ERR) {
	    if (strlen(s))
		SetDlgItemText(hDlg, DEVICE_RES, s);
	    else
	...
	switch (LOWORD(wParam)) {
		...
		SendDlgItemMessage(hDlg, DEVICE_NAME, CB_GETLBTEXT, 
		...
		GetPrivateProfileString(bConvert ? CONVERTSECTION : DEVSECTION,
		...
		SendDlgItemMessage(hDlg, DEVICE_RES, CB_SETCURSEL, 0, 0L);
		if (SendDlgItemMessage(hDlg, DEVICE_RES, CB_GETLBTEXT, 
		...
		return FALSE;
	    case DEVICE_PROP:
		SendDlgItemMessage(hDlg, DEVICE_NAME, WM_GETTEXT, 
		...
		if ( (proplist = get_properties(entry)) 
		    ...
		    free((char *)proplist);
		    nHelpTopic = IDS_TOPICPROP;
		    DialogBoxParam(hlanguage, "PropDlgBox", hDlg, PropDlgProc, 
			(LPARAM)entry);
		}
		else
		    play_sound(SOUND_ERROR);
...
PrintHookProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	SetDlgItemText(hDlg, IDC_GS_DEVNAME, option.printer_device);
	// We can't collate.
	...
	return TRUE;
		...
		return TRUE;
	    case IDC_GS_WINDOWS:
		if (DialogBoxParam(hlanguage, "PrintGDIDlgBox", hDlg, 
			GDIDlgProc, (LPARAM)NULL))
		...
		if (device_queue_list == (char *)NULL)
		    return TRUE;
		if (DialogBoxParam(hlanguage, "PrintGSDlgBox", hDlg, 
		...
		return TRUE;
	    case IDC_GS_ADVPS:
		if (DialogBoxParam(hlanguage, "AdvancedDlgBox", hDlg, 
...
BOOL get_devmode(LPSTR device, HANDLE *hdevmode, HANDLE *hdevnames)
{
    char devicebuf[256];
    char driverbuf[512];
    char *driver;
    ...
    if (hdevnames) {
	length = sizeof(DEVNAMES) + strlen(device) + 1 + 
		strlen(driver) + 1 + strlen(output) + 2;
	if (length < 1024)
	    length = 1024;
	...
	*hdevnames = hglobal;
    ...
    // Obtain the DEVMODE structure from the printer
    if (hdevmode) {
...
BOOL open_printer(void)
{
    ...
    if (!get_devmode(option.printer_queue, &pd.hDevMode, &pd.hDevNames))
	return FALSE;
    ...
    */
    ...
    pd.hDC = CreateDC(driver, device, NULL, lpdevmode);
...
parse_dlgtemplate(LPDLGTEMPLATE pt, LPDLGITEMTEMPLATE *pitem, int item_id)
{
	...
	wsprintf(buf, "style=0x%x exstyle=0x%x cdit=%d x=%d y=%d cx=%d cy=%d\n",
	...
	if (debug_gdi) {
	    wsprintf(buf, "Dialog item %d\n", i);
	    gs_addmess(buf);
	    wsprintf(buf, "  style=0x%x exstyle=0x%x x=%d y=%d cx=%d cy=%d id=%d\n",
    ...
    fixup = (LPBYTE)pw - (LPBYTE)pt;	// length of template
    if (debug_gdi) {
	wsprintf(buf, "Processed %d bytes of DLGTEMPLATE\n", fixup);
	gs_addmess(buf);
	gs_addmess("=== End of dialog template ===\n");
    }
    return fixup;
...
HGLOBAL fix_prn_dlg(void)
{
    ...
    DWORD dwSize = SizeofResource(hCommDlg, hrsrc);
    ...
    if (debug & DEBUG_GDI) {
	wsprintf(buf, "new template size=%ld\n", dwSize + extra_size);
	gs_addmess(buf);
    }
...
BOOL query_printer(void)
{
    ...
    if ((pd.hDevNames == NULL) && (lstrlen(option.printer_queue)))
	get_devmode(option.printer_queue, &pd.hDevMode, &pd.hDevNames);
